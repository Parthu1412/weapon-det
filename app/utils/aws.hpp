...
// redirect retry. Kafka MSK token provider lives in kafka.
...
public:
    ...
    std::optional<std::string> upload_bytes_and_get_url(
        const void* data, size_t size, const std::string& object_name,
        const std::string& content_type = "image/jpeg");

    std::optional<std::string> upload_bytes_and_get_direct_url(
        const void* data, size_t size, const std::string& object_name,
        const std::string& content_type = "image/jpeg")
    {
     ...
     * fallback. Otherwise throws std::invalid_argument. On download failure throws
     * std::runtime_error
     */
