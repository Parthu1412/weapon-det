...
// Convert standard base64 to URL-safe base64 and strip padding (matches Python urlsafe_b64encode().rstrip('='))
...
std::string generate_msk_iam_token(const std::string& region) {
    ...
    Aws::IOStreamFactory streamFactory = []() {
        return Aws::New<Aws::StringStream>("msk_token");
    };
    ...
    Aws::Utils::ByteBuffer buf(reinterpret_cast<const unsigned char*>(signedUri.c_str()), signedUri.size());
