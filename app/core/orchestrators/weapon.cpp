// Weapon Orchestrator - Process for weapon detection.
// Receives frames (+ optional person detections) from Camera Orchestrator via ZMQ.
// Uses WeaponService for business logic: RF-DETR inference, IOU matching, annotation.
// Sends detections to MsgGen Orchestrator via ZMQ.
#include <chrono>
#include <csignal>
#include <cstdlib>
#include <iomanip>
        ...
        if (!cfg.weapon_model_s3_key.empty())
        {
            ...
            } catch (const std::exception& e)
            {
                app::utils::Logger::error(
                    std::string("[WeaponOrche] S3 weapon model download failed: ") + e.what());
                return 1;
            }
        }
        ...
        while (!g_stop)
        {
            ...
            if (pkt.frame.empty())
            {
                app::utils::Logger::warning(
                    "Received None frame camera_id=" + std::to_string(pkt.camera_id) +
                ...
                try
                {
                    ...
                    } else
                    {
                        app::utils::Logger::info(
                            "Sent detection to msg_gen orchestrator | camera_id=" +
                            std::to_string(out.camera_id) +
                            " store_id=" + std::to_string(out.store_id));
                    }
