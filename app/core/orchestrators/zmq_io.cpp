// ZMQ I/O helpers for the weapon pipeline.
// Provides multipart send/recv for two packet types:
//   ZmqWeaponFramePacket  — camera/person-detection → weapon_inference (JSON meta + JPEG frame).
//   ZmqWeaponOutPacket    — weapon_inference → msg_gen (JSON meta + JPEG annotated frame).
// Also exposes make_trace_id() for UUID-style detection trace IDs.
#include "zmq_io.hpp"
...
{
    persons.clear();
    if (!j.contains("person_detections") || !j["person_detections"].is_array())
        return;
    nlohmann::json wrap;
...
bool zmq_send_weapon_output(zmq::socket_t& sock, const ZmqWeaponOutPacket& p)
{
    ...
    {
        if (!cv::imencode(".jpg", p.annotated_frame, jpg, {cv::IMWRITE_JPEG_QUALITY, 95}))
        {
    ...
    try
    {
        ...
        if (j.contains("weapon_detections") && j["weapon_detections"].is_array())
        {
            for (const auto& b : j["weapon_detections"])
            {
                ...
                for (int i = 0; i < 4; ++i)
                    box[i] = b[i].is_number_integer() ? b[i].get<int>()
                                                      : static_cast<int>(b[i].get<double>());
