// Weapon Detection Service - Business logic for weapon detection.
// Handles IOU matching between weapon boxes and person boxes, frame annotation, and decision logic.
// Returns a WeaponProcessResult (weapon boxes, confidence, matched persons, annotated frame)
// or std::nullopt when the detection should not be published.
...
cv::Mat WeaponService::annotate_(const cv::Mat& frame,
                                 const std::vector<std::vector<int>>& weapon_boxes)
{
    ...
    const cv::Mat& frame, const std::vector<app::utils::PersonDetection>* person_detections)
{
    ...
    if (person_detections && !person_detections->empty())
    {
        ...
        {
            if (take[i])
                matched_persons.push_back((*person_detections)[i]);
        }
        app::utils::Logger::info(
            "Weapon detected with IOU match | weapon_count=" + std::to_string(weapon_boxes.size()) +
            " matched_person_count=" + std::to_string(matched_persons.size()));
    } else
    {
