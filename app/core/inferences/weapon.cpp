// Weapon Detection Inference — Pure model call.
// Runs RF-DETR (ONNX Runtime) on a BGR frame; returns weapon bounding boxes
// as [x1, y1, x2, y2] integer pixel coordinates and the max confidence score.
// Does NOT annotate the frame (annotation is done upstream in WeaponService).
#include "weapon.hpp"
...
WeaponInference::WeaponInference(const std::string& onnx_path, float conf_thresh, int input_h,
                                 int input_w, float nms_iou_thresh)
      ...
      input_w_(input_w)
{
    ...
    try
    {
        ...
        try
        {
            ...
            // Force CuDNN to benchmark and find the fastest convolution algorithms for your
            // hardware
        ...
        app::utils::Logger::info(
            "[WeaponInference] Loading ONNX session from " + onnx_path +
    ...
    {
        app::utils::Logger::error(std::string("[WeaponInference] Failed to load model: ") +
                                  e.what());
        std::exit(EXIT_FAILURE);
    }
}
...
void WeaponInference::preprocess_(const cv::Mat& bgr)
{
    ...
    auto raw = torch::from_blob(bgr.data, {1, bgr.rows, bgr.cols, 3}, torch::kByte).to(device);
...
std::pair<std::vector<std::vector<int>>, float> WeaponInference::detect(const cv::Mat& frame)
{
    ...
    try
    {
        ...
        Ort::Value input_tensor =
            Ort::Value::CreateTensor<float>(mem_info, input_buffer_.data(), input_buffer_.size(),
        ...
        auto outputs =
            session_->Run(Ort::RunOptions{nullptr}, input_names, &input_tensor, 1, output_names, 2);
        ...
        for (int64_t i = 0; i < num_queries; ++i)
        {
            ...
            // DETR natively outputs normalized [cx, cy, w, h]
        ...
        // --- NMS with containment suppression ---
        ...
        if (!raw_boxes.empty())
        {
            ...
            for (int i = 0; i < static_cast<int>(nms_indices.size()); ++i)
            {
                ...
                for (int j = i + 1; j < static_cast<int>(nms_indices.size()); ++j)
                {
                    ...
                    const float iy2 =
                        std::min((float)(bi.y + bi.height), (float)(bj.y + bj.height));
                    if (ix2 <= ix1 || iy2 <= iy1)
                        continue;
                    const float inter = (ix2 - ix1) * (iy2 - iy1);
                    const float min_area = std::min(area_i, area_j);
                    // If >= 80% of the smaller box is inside the other, suppress the lower-scored
                    // one
                    if (min_area > 0.f && (inter / min_area) >= 0.8f)
                    {
                        if (scores[static_cast<size_t>(nms_indices[i])] >=
                            scores[static_cast<size_t>(nms_indices[j])])
                            suppressed2[j] = true;
                        else
                            suppressed2[i] = true;
                    }
            ...
            {
                if (!suppressed2[i])
                    final_indices.push_back(nms_indices[i]);
            }
        ...
        for (int idx : nms_indices)
        {
            ...
            int ix2 =
                static_cast<int>(std::lround(std::max(0.f, std::min((float)(r.x + r.width), fw))));
            int iy2 =
                static_cast<int>(std::lround(std::max(0.f, std::min((float)(r.y + r.height), fh))));
        ...
        app::utils::Logger::debug(
            "[WeaponInference] Weapon detection completed"
            " | detection_count=" +
            std::to_string(boxes.size()) + " | confidence_threshold=" +
            std::to_string(conf_thresh_) + " | preprocess=" + std::to_string(ms(t0, t1)) +
