// Person Detection Inference — returns person detections from YOLO TorchScript model.
// Loads the model on GPU (CUDA) if available, falls back to CPU.
// Preprocesses frames with letterbox resize to 640x640 (BGR→RGB, pad 114/255),
// decodes Ultralytics [1, 4+nc, N] output with per-row argmax, applies NMS,
// and filters to person_class_id only — matching Ultralytics classes=[id] behaviour.
#include "person.hpp"
...
PersonInference::PersonInference(const std::string& model_path, float conf_thresh, float iou_thresh,
                                 int person_class_id)
      ...
      person_class_id_(person_class_id)
{
    try
    {
        ...
        app::utils::Logger::info(
            "[PersonInference] Person detection model loaded"
            " | model_path=" +
            model_path + " | person_class_id=" + std::to_string(person_class_id));
    ...
    {
        app::utils::Logger::error(std::string("[PersonInference] Failed to load model: ") +
                                  e.what());
                                      ...
                                      std::vector<app::utils::PersonDetection>& out)
{
    ...
    // [1, 4+nc, N] -> [N, 4+nc]; Ultralytics: assigned class = argmax(cls scores), conf = that
    // score
    ...
    if (num_classes <= 0 || person_class_id_ < 0 || person_class_id_ >= num_classes)
        return;
    ...
    for (int idx : indices)
    {
        ...
        p.box = {static_cast<float>(x1o), static_cast<float>(y1o), static_cast<float>(x2o),
                 static_cast<float>(y2o)};
        p.score = scores[idx];
...
std::vector<app::utils::PersonDetection> PersonInference::detect(const cv::Mat& frame)
{
    ...
    try
    {
        ...
        app::utils::Logger::debug(
            "[PersonInference] Person detection completed"
            " | detection_count=" +
            std::to_string(results.size()) + " | preprocess=" + std::to_string(ms(t0, t1)) +
